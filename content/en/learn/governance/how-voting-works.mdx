...
## A guide to participation in MakerDAO governance

Voting requires [Maker (MKR) tokens](mkr-token.mdx) and takes place in the [Governance Portal](https://vote.makerdao.com/).
...
Please see our guide to [Voting Wallet Setup.](voting-setup.mdx)
...
Setting up a linked-wallet Voting Contract takes four transactions for a total of approximately 1M gas ( typically $1-$2 worth of Ether (ETH)). The cost of setting up a linked-wallet Voting Contract is split between the hot and cold wallets so please ensure both wallets contain Ether (ETH).
...
Governance Polls occur on-chain and are used to measure the sentiment of MKR voters. They can be accessed through the Maker Foundation's [Voting Portal](https://vote.makerdao.com/polling).

[Polls](https://vote.makerdao.com/polling) often run concurrently, allowing voters to participate in any number of them at the same time and use [instant run-off](https://ballotpedia.org/Ranked-choice_voting_(RCV)) so you can select multiple options.

The current schedule calls for polls to 'go live' on a weekly basis every Monday at 12pm EST/9am PST/14:00 UTC.
...
There is a guide on how to do so found [here.](https://forum.makerdao.com/t/how-to-audit-executive-contract-code/1292/7?)
