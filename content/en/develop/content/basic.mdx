...
like React just to produce awesome pages. In the past, the best you could have done 
would be to write content in your editor of choice, export it into markdown 
and then hope that some developer somewhere can incorporate it into the site you're 
working on and add the styles, buttons, images and other visual features which are 
required to make content transformational, rather than being presented as just 
...
Yeah, the weird {} syntax may seem a bit daunting at first, but this is where React components 
begin to become more fun, and powerful, than the normal markdown you're used to. 
This is becuase we can start adding properties to them, or `props` in developer-lingo.
...
of just having a normal icon with you link or an ordinary button, you can do all sorts of funky things:

<Button secondary to={"#"}>secondary</Button> or <Button primaryOutline to={"#"}>primaryOutline</Button> or <Button secondaryOutline to={"#"}>secondaryOutline</Button> or <Button icon={"rocketchat"} to={"#"}>Rocket Chat</Button>

```mdx:title=advanced-buttons.mdx
<Button secondary to={"#"}>secondary</Button> or  

<Button primaryOutline to={"#"}>primaryOutline</Button> or 

<Button secondaryOutline to={"#"}>secondaryOutline</Button> or 

<Button icon={"rocketchat"} to={"#"}>Maker Icon</Button>
```
...
OK, so now we can handle images, links, and buttons - all fo which will make our content really pop 
...
Callout. Sometimes, you don't want to user to take action - you just need to draw attention to a very 
