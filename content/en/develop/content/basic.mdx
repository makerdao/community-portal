...
> When writing relative links, be aware that you can't use the relative dot notation, i.e. `./intermediate`. 
You need to write out the full route from after the locale, i.e. `/develop/content/intermediate`. If you want to 
link to other internal locales, then specify the locale as well, i.e. `es/develop/content/intermediate`

This is because our Link component checks whether it's internal/external, 
if the internal has a locale appened (`fr/work_with_us/bounties`), or if external links have `https` 
(http://google.com will automatically be transformed).
