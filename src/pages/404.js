...
const NotFoundPage = () => {
  ...
  const {allMdx: nodes} = useStaticQuery(graphql`
    query Get404Pages {
      allMdx(filter: {fileAbsolutePath: {regex: "//([\\\\w]{2})/404.mdx$/"}}) {
        nodes {
          headings(depth: h1) {
                  value
                }
  ...
  const page = nodes.nodes.find(({fileAbsolutePath}) => fileAbsolutePath.indexOf(`/${locale}/`) !== -1);
  //Use the Title Rule. Else just use a hardcoded value. 
  const _title = (page !== undefined && page !== null) ? (
                      page.frontmatter.title || (page.headings.length > 0 ? page.headings[0].value : null) || '404: Page Not Found')
                    :
                      '404: Page Not Found';
