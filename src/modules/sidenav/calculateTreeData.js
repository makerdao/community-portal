...
export default (edges, currentTopSection, DEFAULT_LOCALE, currentLocale) => {
  ...
  //by using the filePath and title requirments of an MDX node. 
  	const makeSidenavObjects = (edges, _locale) => {
		...
		return edges.filter(({node}) => node.fileAbsolutePath.indexOf(`/${_locale}/${currentTopSection}/`) !== -1 && node.fileAbsolutePath.indexOf(`/${_locale}/${currentTopSection}/index.mdx`) === -1).flatMap(({node: {headings, frontmatter, fileAbsolutePath}}) => { 
					...
					const slug = fileAbsolutePath.slice(fileAbsolutePath.indexOf(`/${_locale}/`),fileAbsolutePath.length).replace(/(.mdx|index.mdx|.md|.mdx)$/gm,'').replace(/\/$/, "");
  ...
  //BE AWARE(Rejon): This array gets edited in the mergedLocaleFiles map.
	const currentLocaleFiles = DEFAULT_LOCALE !== currentLocale ? makeSidenavObjects(edges, currentLocale) : [];
  ...
  //Reduce all of our mergedLocaleFiles into a object structure that closely resembles our final sidenav.
   ...
   (accu, 
    ...
    for (const part of slicedParts) {
      ...
      if (tmp) { //We've got previous items from our accu
        if (!tmp.items) { //Doesn't have items, give it a blank array.
          tmp.items = [];
        }

        prevItems = tmp.items; //Set prevItems to tmp.items. 
      } else { //We don't have a previous item from our accu. This is a top level directory.
    ...
    const existingItem = prevItems.find(({slugPart}) => slugPart === parts[sliceLength]);
    const test = prevItems;
    
    if (existingItem) { //Set existing item data
      existingItem.url = slug; 
      existingItem.title = title;
    }
    else { //No existing item, push this as a new prevItem. 
      prevItems.push({
