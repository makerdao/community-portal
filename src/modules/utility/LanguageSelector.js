...
const LanguageSelector = () => {
  ...
  `);	
  const pageLocaleRegex = new RegExp(`(/([\\w]{2})/${pathnameStripped})/([^/]+)$`, 'gm')
  const existingLanguages = languagePages.edges.filter(({node}) => pageLocaleRegex.test(node.fileAbsolutePath) && !node.fileAbsolutePath.includes(`/${locale}/`)).map(({node}) => {
	  const value = UrlConverter(node);
	  const _locale = node.fileAbsolutePath.slice(node.fileAbsolutePath.indexOf("/content/") + 8, node.fileAbsolutePath.indexOf("/content/") + 11).replace(/^\//g, "");
	...
	const MenuList = props => {
		return (
			<>
				...
				<Box sx={{position: 'relative', p: 2, pl: '12px', pr: '12px', '::before': {
					content: '""',
					height: '1px',
					width: '90%',
					position: 'absolute',
	...
	if (existingLanguages.length > 0) {
		...
		return (
			<>
				<Select sx={{width: '262px'}} theme={selectTheme => ({...selectTheme, fontFamily: theme.fonts.body, colors: {...selectTheme.colors, ...uiSelectTheme}})} components={{MenuList}} options={existingLanguages} onChange={onChange} aria-label={t('Page_Language_Selector')} value={{
					value: pathname, 
					label: t("Language")
				}}/>
			</>
	...
	return (
		...
		<Text sx={{mb: 2, letterSpacing:'.7rem'}}>{allLocales.filter((loc) => loc !== locale).map((loc) => (t('Flag', null, null, loc)))}</Text>
