...
const Breadcrumbs = ({ children }) => {
  ...
  const {allMdx} = useStaticQuery(graphql`
    query GetBreadcrumbsTitles {
      allMdx(filter: {fileAbsolutePath: {regex: "//([\\\\w]{2})/(?!header.mdx|index.mdx|404.mdx)/"}}) {
        edges {
          node {
            headings(depth: h1) {
              value
  ...
  edges = edges.filter(({node}) => node.fileAbsolutePath.indexOf(`/${locale}/`) !== -1)
  ...
  const BreadcrumbData = pathDirs.map((pathDir) => {
    ...
    const {node} = edges.find(({node}) => pathDir === node.fileAbsolutePath.replace(/(.mdx|\/index.mdx|.md)$/gm, '').split('/').pop())
    ...
    const title = node.frontmatter.title || (node.headings.length > 0 ? node.headings[0].value : null) || url.split('/').slice(-2, -1)[0];
