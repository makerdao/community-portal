...
const Accordion = ({children, defaultOpen, openIcon, closeIcon}) => {
	...
	return (
		<Box sx={{
			...
			<Flex sx={{justifyContent: 'space-between', cursor: 'pointer', position: 'relative'}} onClick={() => setExpanded(!expanded)}>
				 ...
				 */}
				{!expanded ?
					<Icon name={expanded ? CloseIcon : OpenIcon} color={'headline'} size={4} />
					:
					<Box sx={{fontSize: '3rem', width: '32px', height: '32px', textAlign: 'center', lineHeight: '32px', fontWeight: 'bold'}}>-</Box>
				}
			</Flex>
			<SmoothCollapse eagerRender={true} allowOverflowWhenOpen={true} expanded={expanded}>
