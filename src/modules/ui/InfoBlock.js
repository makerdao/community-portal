...
const InfoBlock = ({children, alt, backgroundImage, minImageWidth, sx}) => {
	...
	const getImageData = () => {
		...
		const getElementMinSize = (img) => {
			...
			//If we're using MD image scripting, it will be a little deeper.
				...
				if (img.props.children.props.className === 'gatsby-resp-image-wrapper') {
		...
		if (checkIfElementIsImage(_Children[0])) {
			...
			return {img, first: true, minWidth: minImageWidth || getElementMinSize(img)};
		}
			...
			return {img, first: false, minWidth: minImageWidth || getElementMinSize(img)}
		} 
		else {
	...
	const NoImageStyles = imageData === null ? {
		width: '100%',
		textAlign: 'center',
	...
	return (
		<Flex sx={{justifyContent: 'space-evenly', alignItems: 'center', background: `url(${backgroundImage})`,...sx}}>	
			{imageData &&
				<Box sx={{order: imageData.first ? 0 : 1, flex: alt ? '1' : '1 0 0%', minWidth: alt ? '50%' : imageData.minWidth, maxWidth: alt ? '50%' : '40%', '& > *': {m: 0}, '& .gatsby-resp-image-wrapper': alt ? {maxWidth: 'unset !important', width: '100%'} : {}}}>
					{imageData.img}
				</Box>
			}
			<Box sx={{order: (imageData && imageData.first) ? 1 : 0, width: 'auto', margin: 'auto 0', pr: '16px', ...NoImageStyles}}>
